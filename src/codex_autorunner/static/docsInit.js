...
export function initDocs() {
    ...
    const reloadDocBtn = document.getElementById("reload-doc");
    if (reloadDocBtn) {
    ...
    const ingestSpecBtn = document.getElementById("ingest-spec");
    if (ingestSpecBtn) {
    ...
    if (specIngestUI.patchReload)
        specIngestUI.patchReload.addEventListener("click", () => reloadSpecIngestPatch(false));
    if (docActionsUI.copy) {
        docActionsUI.copy.addEventListener("click", () => copyDocToClipboard());
    }
    ...
    if (threadRegistryUI.download) {
        threadRegistryUI.download.addEventListener("click", downloadThreadRegistryBackup);
    }
            ...
            if (e.key === "ArrowUp") {
                ...
                if ((isEmpty || atStart) && state.history.length > 0) {
                    ...
                    if (getHistoryNavIndex() < maxIndex) {
                        ...
                        const entry = state.history[getHistoryNavIndex()];
