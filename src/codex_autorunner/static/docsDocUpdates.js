...
import { getActiveDoc, docsState, hasDraft, isDraftPreview, } from "./docsState.js";
...
export async function applyDocUpdateFromChat(kind, content, { force = false } = {}) {
    ...
    if (viewingSameDoc && textarea) {
        ...
        if (!force && !previewing && textarea.value !== cached) {
            const ok = await confirmModal(`You have unsaved ${kind.toUpperCase()} edits. Overwrite with chat result?`);
            if (!ok) {
                flash(`Kept your unsaved ${kind.toUpperCase()} edits; chat result not applied.`);
                return false;
            }
        }
    }
