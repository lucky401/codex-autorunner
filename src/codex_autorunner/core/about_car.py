...
def build_about_car_markdown(
    *,
    ...
    return (
        ...
        "- **Spec**: "
        f"`{spec_disp}`\n"
        "- Migration note: if this repo is moving to `.codex-autorunner/contextspace/*`, use the configured docs paths from config and do not keep duplicate workspace/contextspace copies.\n\n"
...
def build_ticket_flow_quickstart_markdown(*, repo_root: Path) -> str:
    ...
    return (
        ...
        "- Self-check CLI availability:\n"
        "  `car --help` (or `command -v car`)\n"
        "- If `car` is not on PATH:\n"
        "  1) Run `make setup`\n"
        "  2) Use `.venv/bin/car ...`\n"
        "  3) Or use `python -m codex_autorunner.cli ...`\n"
        "- Bootstrap the first run (creates TICKET-001 if needed):\n"
        "  `car flow ticket_flow bootstrap --repo <path>`\n"
        "  Fallbacks: `.venv/bin/car flow ticket_flow bootstrap --repo <path>` or `python -m codex_autorunner.cli flow ticket_flow bootstrap --repo <path>`\n"
        ...
        "- Path transition note: some repos may migrate docs from `.codex-autorunner/workspace/*` to `.codex-autorunner/contextspace/*`; follow configured docs paths and avoid creating duplicate copies.\n"
