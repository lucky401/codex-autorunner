...
export interface DocChatVoiceConfig {
  buttonId: string;
  inputId: string;
  statusId?: string;
}
...
export async function initDocChatVoice(config: DocChatVoiceConfig): Promise<void> {
  const voiceBtn = document.getElementById(config.buttonId) as HTMLButtonElement | null;
  const input = document.getElementById(config.inputId) as HTMLTextAreaElement | null;
  const statusEl = config.statusId
    ? (document.getElementById(config.statusId) as HTMLElement | null)
    : null;
