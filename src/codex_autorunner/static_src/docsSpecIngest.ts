...
export function renderSpecIngestPatch(): void {
...
async function interruptSpecIngest(): Promise<void> {
  try {
    await api("/api/ingest-spec/interrupt", { method: "POST" });
  } catch (err) {
    flash((err as Error).message || "Failed to interrupt spec ingest", "error");
  }
}

export async function ingestSpec(): Promise<void> {
  if (docsState.specIngestState.busy) return;
  const needsForce = ["todo", "progress", "opinions"].some(
    (k) => (docsState.docsCache[k as "todo" | "progress" | "opinions"] || "").trim().length > 0
  );
  if (needsForce) {
  ...
  const button = document.getElementById("ingest-spec") as HTMLButtonElement | null;
...
export function cancelSpecIngest(): void {
  if (!docsState.specIngestState.busy) return;
  interruptSpecIngest();
...
export async function continueSpecIngest(): Promise<void> {
  if (docsState.specIngestState.busy) return;
...
export async function applySpecIngestPatch(): Promise<void> {
  ...
  try {
    ...
    flash((err as Error).message || "Failed to apply spec ingest patch", "error");
  } finally {
    docsState.specIngestState.busy = false;
    renderSpecIngestPatch();
  }
...
export async function discardSpecIngestPatch(): Promise<void> {
  ...
  try {
    ...
    flash((err as Error).message || "Failed to discard spec ingest patch", "error");
  } finally {
    docsState.specIngestState.busy = false;
    renderSpecIngestPatch();
  }
...
export async function reloadSpecIngestPatch(silent = false): Promise<void> {
  try {
