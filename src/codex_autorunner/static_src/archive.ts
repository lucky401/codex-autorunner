...
import {
  ...
  listArchiveTree,
  listLocalArchiveTree,
  listLocalRunArchives,
  readArchiveFile,
...
let localArchives: LocalRunArchiveSummary[] = [];
type ArchiveListItem =
  | { kind: "snapshot"; summary: ArchiveSnapshotSummary }
  | { kind: "local"; summary: LocalRunArchiveSummary };
let selectedItem: ArchiveListItem | null = null;
let activeSourceKey = "";
...
function archiveSignature(
  snapshotItems: ArchiveSnapshotSummary[],
  localItems: LocalRunArchiveSummary[]
): string {
...
type ArchiveFileState = {
  ...
  selectedFile: ArchiveTreeNode | null;
};
...
type ArchiveFileEls = {
  ...
  fileEmpty: HTMLElement;
  downloadBtn: HTMLButtonElement;
  loadBtn: HTMLButtonElement;
  quickLinks: HTMLElement | null;
  refreshBtn: HTMLButtonElement | null;
};

let fileState: ArchiveFileState | null = null;
...
const LOCAL_QUICK_LINKS: Array<{ label: string; path: string; kind: "file" | "folder" }> = [
...
function listItemKey(item: ArchiveListItem): string {
...
function renderSubTabs(summaryLabel: string): string {
  return `
    <div class="archive-subtabs">
      <button class="archive-subtab${activeSubTab === "snapshot" ? " active" : ""}" data-subtab="snapshot">${escapeHtml(
        summaryLabel
      )}</button>
...
function switchSubTab(tab: "snapshot" | "files"): void {
  activeSubTab = tab;
  // Update tab button states
  const tabBtns = document.querySelectorAll(".archive-subtab");
  tabBtns.forEach((btn) => {
    const btnTab = (btn as HTMLElement).dataset.subtab;
...
function wireSubTabs(): void {
  ...
  container.addEventListener("click", (e) => {
    ...
    if (!target) return;
    const btn = target.closest(".archive-subtab") as HTMLElement | null;
    if (!btn) return;
    const tab = btn.dataset.subtab as "snapshot" | "files" | undefined;
...
function renderFileSection(
  quickLinksData: Array<{ label: string; path: string; kind: "file" | "folder" }>,
  description: string
): string {
  const quickLinks = quickLinksData
    .map(
      (item) =>
        `<button class="ghost sm" data-archive-path="${escapeHtml(item.path)}" data-archive-kind="${item.kind}">${escapeHtml(item.label)}</button>`
    )
    .join("");
...
function renderLocalSummary(run: LocalRunArchiveSummary): string {
...
function collectFileEls(): ArchiveFileEls | null {
  ...
  const downloadBtn = document.getElementById("archive-file-download") as HTMLButtonElement | null;
  const loadBtn = document.getElementById("archive-file-load") as HTMLButtonElement | null;
  const quickLinks = document.getElementById("archive-quick-links");
  const refreshButton = document.getElementById("archive-tree-refresh") as HTMLButtonElement | null;
...
function renderFileList(): void {
  ...
  fileState.nodes.forEach((node) => {
    ...
    if (fileState.selectedFile?.path === node.path) row.classList.add("active");
    row.tabIndex = 0;
...
async function listTreeForState(state: ArchiveFileState, path: string): Promise<{ path: string; nodes: ArchiveTreeNode[] }> {
...
async function readFileForState(state: ArchiveFileState, path: string): Promise<string> {
...
function downloadFileForState(state: ArchiveFileState, path: string): void {
  if (state.kind === "snapshot" && state.snapshotId) {
...
async function navigateTo(path: string): Promise<void> {
  ...
  try {
    ...
    if (!fileState || fileState.sourceKey !== activeSourceKey) return;
    if (requestId !== treeRequestToken) return;
    fileState.currentPath = res.path || "";
    fileState.nodes = res.nodes || [];
    renderFileList();
  } catch (err) {
    fileEls.list.innerHTML = "";
    const msg = (err as Error).message || "Failed to load archive tree";
...
async function selectFile(node: ArchiveTreeNode, forceLoad = false): Promise<void> {
  if (!fileState || !fileEls) return;
  ...
  try {
    ...
    if (!fileState || fileState.sourceKey !== activeSourceKey) return;
    if (requestId !== fileRequestToken) return;
    let display = text;
    ...
    const msg = (err as Error).message || "Failed to load file";
    fileEls.fileContent.textContent = msg;
    flash("Failed to load archive file.", "error");
  }
}

function initArchiveFileViewer(source: ArchiveListItem): void {
...
function wireArchivePathButtons(container: HTMLElement | null): void {
  ...
  container.addEventListener("click", (event) => {
    ...
    const kind = btn.dataset.archiveKind as "file" | "folder" | undefined;
    if (!path) return;
    if (kind === "folder") {
...
async function loadLocalDetail(target: LocalRunArchiveSummary): Promise<void> {
  if (!detailEl) return;
...
function selectItem(target: ArchiveListItem): void {
  selectedItem = target;
...
async function loadArchiveData(forceReload = false): Promise<void> {
  if (!listEl) return;
  ...
  try {
    ...
    if (!sortedSnapshots.length && !sortedLocals.length) return;

    const selectedKey = selectedItem ? listItemKey(selectedItem) : "";
    const matchSnapshot =
      selectedKey && selectedKey.startsWith("snapshot:")
        ? sortedSnapshots.find((item) => `snapshot:${snapshotKey(item)}` === selectedKey)
        : null;
    const matchLocal =
      selectedKey && selectedKey.startsWith("local:")
