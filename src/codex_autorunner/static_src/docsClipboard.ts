...
import type { DocKind } from "./docsState.js";

export async function copyDocToClipboard(kind: DocKind = getActiveDoc()): Promise<void> {
...
export async function pasteSpecFromClipboard(): Promise<void> {
  const activeDoc = getActiveDoc();
  if (!PASTEABLE_DOCS.includes(activeDoc as "spec")) return;
