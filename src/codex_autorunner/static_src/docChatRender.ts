...
export function updatePatchPreviewFromDraft(draft: DraftPayload | undefined): void {
  if (!chatUI.patchBody || !draft || !draft.patch) return;
...
export function renderChatHistory(state: ChatState): void {
  ...
  state.history.slice(0, CHAT_HISTORY_LIMIT).forEach((entry: ChatHistoryEntry) => {
    ...
    copyBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      if (chatUI.input) chatUI.input.value = entry.prompt;
